...
cdef extern from "rbd/librbd.h" nogil:
    ...
    int rbd_get_modified_timestamp(rbd_image_t image, timespec *timestamp)
...
written." % (self.name, ret, length))
    ...
    def modified_timestamp(self):
        """
        Return the modified timestamp for the image.
        """
        cdef:
            timespec timestamp
        with nogil:
            ret = rbd_get_modified_timestamp(self.image, &timestamp)
        if ret != 0:
            raise make_ex(ret, 'error getting modified timestamp for image: %s' % (self.name))
        return datetime.fromtimestamp(timestamp.tv_sec)
